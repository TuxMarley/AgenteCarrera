...
export async function GET(request: Request) {
  try {
    ...
    const result = await rawDb().prepare(`SELECT id, title, description, status, started_at AS startedAt, completed_at AS completedAt, created_at AS createdAt, updated_at AS updatedAt FROM work_activities WHERE user_id = ? ORDER BY updated_at DESC`).bind(actor.id).all();
...
export async function POST(request: Request) {
  try {
    ...
    const activity = { id, title, description, status, startedAt:timestamp, completedAt:status === 'completed' ? timestamp : null, createdAt:timestamp, updatedAt:timestamp };
    const db = rawDb();
    await db.batch([
      db.prepare(`INSERT INTO work_activities (id, user_id, title, description, status, started_at, completed_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, actor.id, title, description, status, timestamp, activity.completedAt, timestamp, timestamp),
