...
export async function getDashboard(userId: string, includePrivateReview = false) {
  ...
  const db = rawDb();
  const [userRow, declaredProfileRow, competencyRows, actionRows, evidenceRows, activityRows, historyRows, profileAnalysisRow] = await Promise.all([
    ...
    db.prepare(`SELECT id, title, description, status, started_at AS startedAt, completed_at AS completedAt, created_at AS createdAt, updated_at AS updatedAt FROM work_activities WHERE user_id = ? ORDER BY updated_at DESC`).bind(userId).all(),
  ...
  return { profile, competencies: competencyRows.results, actions: actionRows.results, evidence: evidenceRows.results, workActivities: activityRows.results, history: historyRows.results, profileAnalysis };
}

export async function getWorkActivities(userId: string) {
  await ensureDatabase();
  const result = await rawDb().prepare(`SELECT id, title, description, status, started_at AS startedAt, completed_at AS completedAt, created_at AS createdAt, updated_at AS updatedAt FROM work_activities WHERE user_id = ? ORDER BY updated_at DESC`).bind(userId).all();
